...
class AnimalTableViewController: UITableViewController {
    ...
    var animalsDic = [String: [String]] ()
    var animalsSectionTitles = [String]()
    
    let animalIndexTitle = ["A","B","C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "Ñ", "O", "P", "Q", "R", "S", "T", "U","V", "X", "Y","Z"]
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        guard let animalValues = animalsDic[animalKey] else {
            return 0
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       /* let imageFileName = animals[indexPath.row].lowercased().replacingOccurrences(of: " ", with: "_")
        ...
        if let animalValues = animalsDic[animalKey]{
            cell.textLabel?.text = animalValues[indexPath.row]
            let imagesFilName = animalValues[indexPath.row].lowercased().replacingOccurrences(of: "", with: "_")
            cell.imageView?.image = UIImage(named: imagesFilName)
        }
    ...
    override func tableView(_ tableView: UITableView, sectionForSectionIndexTitle title: String, at index: Int) -> Int {
        guard let index = animalsSectionTitles.index(of: title)
            else{
                return -1
        }
        return index
    ...
    func createAnimalDict(){
        for animal in animals{
            ...
            if var animalValues = animalsDic[animalKey] {
                animalValues.append(animal)
                animalsDic[animalKey] = animalValues
                
            }else{
                animalsDic[animalKey] = [animal]
            }
        }
        animalsSectionTitles = [String](animalsDic.keys)
        animalsSectionTitles = animalsSectionTitles.sorted(by:{
            $0 < $1} )
        //ordenamiento burbuja (by:{$0 < $1} )
    }
